...
def encrypt(msg):
    ...
    print 'encrypt: %s p = %s, alpha = %s, alpha^a = %s, pubkey = %s'\
        % (msg, pub_key[0], pub_key[1], pub_key[2], pub_key)
    b = 1115
    c = []
...
def check(sign, msg):
    ...
    if(p % sign[0] == 0):
        ...
        return
    _d = (pow(pub_key[2], sign[0], pub_key[0])
          * pow(sign[0], sign[1], pub_key[0])) % pub_key[0]
